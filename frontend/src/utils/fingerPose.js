...
const L = 0, TIP = 1, PIP = 2, MCP = 3, CMC = 4;
...
export function classifyLetter(fingers, lm) {
  ...
  if (count >= 4) {
    if (thumb && index && middle && ring && pinky) {
      if (tipDists.ti < 0.06 && tipDists.tm < 0.06) return { label: "O", confidence: 0.85, top3: [["O", 0.85], ["C", 0.1], ["B", 0.05]] };
      return { label: "B", confidence: 0.8, top3: [["B", 0.8], ["O", 0.15], ["C", 0.05]] };
    }
    if (!thumb && index && middle && ring && pinky) return { label: "B", confidence: 0.85, top3: [["B", 0.85], ["4", 0.1], ["C", 0.05]] };
    if (thumb && index && middle && ring && !pinky) {
      if (tipDists.ti < 0.08) return { label: "O", confidence: 0.6, top3: [["O", 0.6], ["W", 0.3], ["C", 0.1]] };
      return { label: "W", confidence: 0.7, top3: [["W", 0.7], ["K", 0.15], ["?4", 0.15]] };
    }
    return { label: "?", confidence: 0.3, top3: [["?", 0.3], ["B", 0.2], ["W", 0.2]] };
  }

  if (count === 3) {
    if (index && middle && ring && !thumb && !pinky) return { label: "W", confidence: 0.75, top3: [["W", 0.75], ["K", 0.15], ["?3", 0.1]] };
    if (thumb && index && middle && !ring && !pinky) return { label: "K", confidence: 0.7, top3: [["K", 0.7], ["W", 0.15], ["L", 0.15]] };
    if (thumb && index && pinky && !middle && !ring) return { label: "I", confidence: 0.5, top3: [["I", 0.5], ["Y", 0.3], ["L", 0.2]] };
    return { label: "?", confidence: 0.3, top3: [["?", 0.3], ["K", 0.2], ["W", 0.2]] };
  }

  if (count === 2) {
    if (thumb && pinky && !index && !middle && !ring) return { label: "Y", confidence: 0.9, top3: [["Y", 0.9], ["L", 0.05], ["I", 0.05]] };
    if (thumb && index && !middle && !ring && !pinky) return { label: "L", confidence: 0.85, top3: [["L", 0.85], ["Y", 0.1], ["?2", 0.05]] };
    if (index && middle && !thumb && !ring && !pinky) {
      if (tipDists.im > 0.08) return { label: "V", confidence: 0.8, top3: [["V", 0.8], ["U", 0.15], ["?2", 0.05]] };
      return { label: "U", confidence: 0.7, top3: [["U", 0.7], ["V", 0.2], ["H", 0.1]] };
    }
    if (index && ring && !thumb && !middle && !pinky) return { label: "H", confidence: 0.4, top3: [["H", 0.4], ["U", 0.3], ["V", 0.3]] };
    if (thumb && middle && !index && !ring && !pinky) return { label: "T", confidence: 0.4, top3: [["T", 0.4], ["K", 0.3], ["?2", 0.3]] };
    return { label: "?", confidence: 0.3, top3: [["?", 0.3], ["Y", 0.2], ["L", 0.2]] };
  }

  if (count === 1) {
    if (index) return { label: "D", confidence: 0.85, top3: [["D", 0.85], ["L", 0.05], ["X", 0.05]] };
    if (pinky) return { label: "I", confidence: 0.8, top3: [["I", 0.8], ["Y", 0.1], ["L", 0.05]] };
    if (middle) return { label: "?1M", confidence: 0.3, top3: [["?1M", 0.3], ["U", 0.25], ["V", 0.25]] };
    if (ring) return { label: "?1R", confidence: 0.2, top3: [["?1R", 0.2], ["W", 0.2], ["?1", 0.2]] };
    if (thumb) {
      const tipY = lm[4].y;
      const ipY = lm[3].y;
      const mcpY = lm[1].y;
      if (tipY < ipY && tipY < mcpY) return { label: "L", confidence: 0.5, top3: [["L", 0.5], ["Y", 0.3], ["D", 0.2]] };
      return { label: "?", confidence: 0.3, top3: [["?", 0.3], ["A", 0.2], ["T", 0.2]] };
    }
  }
  ...
  if (thumbOver) {
    ...
    if (toPinkyDist < toRingDist && toPinkyDist < 0.06) return { label: "M", confidence: 0.5, top3: [["M", 0.5], ["N", 0.3], ["A", 0.2]] };
    if (toRingDist < toMidDist && toRingDist < 0.06) return { label: "N", confidence: 0.5, top3: [["N", 0.5], ["M", 0.3], ["A", 0.2]] };
    return { label: "S", confidence: 0.5, top3: [["S", 0.5], ["A", 0.3], ["T", 0.2]] };
  }

  if (thumbAcross) return { label: "A", confidence: 0.6, top3: [["A", 0.6], ["S", 0.2], ["T", 0.2]] };
