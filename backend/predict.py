...
pipeline = joblib.load(MODEL_PATH or "frontend/public/model/vsl_model.joblib")
...
for line in sys.stdin:
    ...
    if features is None or len(features) != scaler.n_features_in_:
        print(json.dumps({
            "error": f"Expected {scaler.n_features_in_} features, got {len(features) if features else 0}"
        }))
        continue
